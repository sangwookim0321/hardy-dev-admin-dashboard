...
<script>
	...
	async function getItem() {
		await httpGet(
			`${endPoints.SUBJECTIVE_TEST}/?creation_date=${selectedCreationDate}&views=${selectedViews}&disclosure=${selectedDisclosure}&search=${searchKeyword}&page=${currentPage}&limit=${pageSize}`,
			'subjectiveTestList',
			true,
			(res) => {
				items = res.data.data
	...
	async function deleteItem() {
		...
		await httpDelete(
			endPoints.SUBJECTIVE_TEST,
			'subjectiveTestDelete',
	...
	function updatePage(page) {
		currentPage = page
		getItem()
		goto(`/admin/subjectiveTest/list?page=${currentPage}`, { replaceState: true })
		checkboxStatus = []
	}
	...
	async function moveDetail(id) {
		await goto(`/admin/subjectiveTest/list/${id}?page=${currentPage}`)
	}
...
{:else}
	<main>
		<div class="main_top_box">
			...
			<span style="color: #fd7e14">주관식 테스트 목록</span>
		</div>
		<div class="main_box">
...
<style>
	...
	.search_box button:hover {
		background-color: var(--main-bg-darkOrange);
	}
	.select_div {
		position: relative;
	}
